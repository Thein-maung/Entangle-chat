...
export async function encryptAudioChunk(chunk, chunkSize = 1024) {
  const pad = await nextPad(chunkSize);
  const encrypted = new Uint8Array(chunk.length);
  for (let i = 0; i < chunk.length; i++) encrypted[i] = chunk[i] ^ pad[i % chunkSize];
  return encrypted;
}

export async function decryptAudioChunk(encryptedChunk, chunkSize = 1024) {
  const pad = await nextPad(chunkSize);
  const decrypted = new Uint8Array(encryptedChunk.length);
  for (let i = 0; i < encryptedChunk.length; i++) decrypted[i] = encryptedChunk[i] ^ pad[i % chunkSize];
  return decrypted;
       }
